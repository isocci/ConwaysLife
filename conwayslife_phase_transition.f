...
34 constant n 				           { Life is simulated on a nxn grid, with a frame of 0s making an absorbing boundary }
...
80 constant probability			{ probability (out of 100) that Life rules will be applied to each cell }
...
make_array_ran life_matrix       		{ creates initial main life matrix }
	...
	n m * m + m do	                                          		{ looping through the nxn grid for LIFE and ignoring the frame }
		n 0 do
		  ...
		  { --- Applying rules of LIFE --- }
			...
			100 RND probability < if 			 { each cell has a probability of being updated with Life rules }
				{ --- applying rules of LIFE --- }
				current_cell @ c@ 1 = if
					sum @ 2 <> sum @ 3 <> AND if
						0 current_cell @ c!
